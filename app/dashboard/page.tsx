      ...
      <div style={{ background: '#fff', borderRadius: '16px 16px 0 0', padding: '28px 24px', width: '100%', maxWidth: 480, fontFamily: 'Georgia, serif', maxHeight: '90vh', overflowY: 'auto' }}>
        {step === 'loading' && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <div style={{ color: '#8a7c6a' }}>Loading your prayer network...</div>
          </div>
        )}
        {step === 'sending' && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <div style={{ fontSize: 32, marginBottom: 12 }}>✝</div>
            <div style={{ color: '#8a7c6a' }}>Sending your prayer request...</div>
          </div>
        )}
        {step === 'sent' && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>🙏</div>
            <div style={{ fontSize: 20, fontWeight: 'bold', color: '#1a1208', marginBottom: 8 }}>Prayer Request Sent</div>
            <div style={{ fontSize: 14, color: '#8a7c6a' }}>
              {sentCount} {sentCount === 1 ? 'person is' : 'people are'} standing with you in prayer. ✝
            </div>
          </div>
        )}
        {step === 'error' && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <button onClick={onClose} style={{ padding: '10px 24px', borderRadius: 8, border: '1px solid #ddd', background: '#fff', cursor: 'pointer', fontFamily: 'Georgia, serif' }}>Close</button>
          </div>
        )}
        {step === 'compose' && (
          <>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <textarea
                ...
                rows={4}
                style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd6ca', fontSize: 15, fontFamily: 'Georgia, serif', background: '#fdfaf7', color: '#2c2416', resize: 'none', boxSizing: 'border-box' as const }}
              />
            </div>
            ...
            {network.length === 0 ? (
              <div style={{ background: '#f7f4ef', borderRadius: 10, padding: 16, marginBottom: 20, fontSize: 13, color: '#8a7c6a', textAlign: 'center' }}>
                No one in your prayer network yet. Share bands with people to build your network.
              </div>
            ) : (
              <div style={{ marginBottom: 20 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#7a6c5a', letterSpacing: 0.5, textTransform: 'uppercase' as const, display: 'block', marginBottom: 8 }}>
                  Send To ({network.length - excluded.length} of {network.length})
                </label>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {network.map((person) => {
                    ...
                    return (
                      <div
                        key={person.email}
                        onClick={() => toggleExclude(person.email)}
                        style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: isExcluded ? '#f7f4ef' : '#f0f7f3', borderRadius: 8, cursor: 'pointer', border: `1px solid ${isExcluded ? '#e8e1d6' : '#c8e6d4'}`, opacity: isExcluded ? 0.6 : 1 }}
                      >
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={send} disabled={!message.trim()} style={{ flex: 2, padding: '12px', borderRadius: 8, border: 'none', background: message.trim() ? AMBER : '#ddd', color: '#fff', fontSize: 15, fontWeight: 'bold', cursor: 'pointer', fontFamily: 'Georgia, serif' }}>
                Send Prayer Request ✝
              </button>
            </div>
          </>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #f0ece6' }}>
        {(['others', 'mine'] as const).map(t => (
          ...
          >
            ...
            {t === 'others' && others.length > 0 && <span style={{ marginLeft: 5, background: AMBER, color: '#fff', borderRadius: 10, padding: '1px 6px', fontSize: 10 }}>{others.length}</span>}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: '10px 16px', borderTop: '1px solid #f0ece6', textAlign: 'right' }}>
        <button onClick={() => {}} style={{ fontSize: 12, color: AMBER, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'Georgia, serif' }}>
          View all in Prayer List →
        </button>
      </div>
    </div>
  )
  ...
  .order('created_at', { ascending: false })
        ...
        if (myBands.length > 0) {
          ...
          const pts = (regsData || [])
            ...
            .map((r) => ({ lat: r.latitude, lng: r.longitude, band_id: r.band_id, user_name: r.user_name, city: r.city, country: r.country, prayer: r.prayer }))
          setMapPoints(pts)
    ...
    if (activeTab === 'Map')
      return (
        <div>
          ...
          <p style={{ fontSize: 14, color: '#8a7c6a', marginBottom: 20 }}>
            {mapPoints.length} location{mapPoints.length !== 1 ? 's' : ''} across {stats.countries} countr{stats.countries !== 1 ? 'ies' : 'y'}.
          </p>
    ...
    <div style={{ minHeight: '100vh', background: '#f7f4ef', fontFamily: 'Georgia, serif', color: '#2c2416' }}>
      <div style={{ background: AMBER, color: '#fff', display: 'flex', alignItems: 'center', padding: '0 16px', height: 56, gap: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.15)', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <button
          ...
          style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: '#fff', padding: '6px 12px', borderRadius: 6, cursor: 'pointer', fontSize: 13, fontFamily: 'Georgia, serif' }}
        >
          Sign out
        </button>
      </div>
        ...
        <div style={{ background: '#fff', borderBottom: '1px solid #e8e1d6', padding: '0 32px', display: 'flex', gap: 4 }}>
          {TABS.map((t) => (
            <button
              ...
              style={{ padding: '14px 18px', border: 'none', borderBottom: activeTab === t ? `2px solid ${AMBER}` : '2px solid transparent', background: 'transparent', color: activeTab === t ? AMBER : '#5a4f42', fontSize: 14, fontWeight: activeTab === t ? 700 : 400, cursor: 'pointer', fontFamily: 'Georgia, serif' }}
            >
              {t}
            </button>
          ))}
          ...
          {TABS.map((item) => {
            ...
            return (
              <button
                ...
                style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '10px 2px 8px', border: 'none', background: 'transparent', cursor: 'pointer', position: 'relative', minWidth: 0 }}
              >
