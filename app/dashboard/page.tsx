...
export default function Dashboard() {
  ...
  async function openPrayerModal() {
    ...
    const res = await fetch('/api/prayer-network?uid=' + user.id)
    ...
    <div style={{ fontFamily: 'Georgia, serif', background: '#FDFAF5', minHeight: '100vh', color: '#2C1A0E' }}>
            ...
            <div className="main-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28 }}>
              <div>
                ...
                <div style={{ display: 'grid', gap: 12 }}>
                  ...
                  <button onClick={openPrayerModal} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '18px 20px', background: 'linear-gradient(135deg, #1a6b4a, #2d9966)', border: 'none', borderRadius: 8, cursor: 'pointer', width: '100%', textAlign: 'left' as const }}>
      ...
      {showPrayerModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 24 }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: '36px 40px', maxWidth: 520, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', fontFamily: 'Georgia, serif', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {prayerResult ? (
              <div style={{ textAlign: 'center' }}>
                ...
                <button onClick={() => { setShowPrayerModal(false); setPrayerRequest('') }} style={{ marginTop: 20, padding: '10px 28px', background: '#1a6b4a', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, cursor: 'pointer', fontFamily: 'Georgia, serif' }}>Close</button>
              </div>
            ) : (
              <div>
                ...
                {/* Network list */}
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#7a6c5a', display: 'block', marginBottom: 8, letterSpacing: 0.4 }}>SEND TO (uncheck to exclude)</label>
                  {networkLoading ? (
                    <div style={{ fontSize: 13, color: '#9B7B62', padding: '12px 0' }}>Loading your network...</div>
                  ) : network.length === 0 ? (
                    <div style={{ fontSize: 13, color: '#9B7B62', fontStyle: 'italic', padding: '12px 0' }}>No one in your network yet. Give bands to people to build your prayer network.</div>
                  ) : (
                    <div style={{ border: '1px solid #e8e1d6', borderRadius: 8, overflow: 'hidden' }}>
                      {network.map((person, i) => (
                        <div key={person.email} onClick={() => toggleExclude(person.email)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: i < network.length - 1 ? '1px solid #f7f4ef' : 'none', cursor: 'pointer', background: excludedEmails.includes(person.email) ? '#fef9f0' : '#fff', transition: 'background 0.15s' }}>
                          <div style={{ width: 20, height: 20, borderRadius: 4, border: '2px solid ' + (excludedEmails.includes(person.email) ? '#ddd6ca' : '#1a6b4a'), background: excludedEmails.includes(person.email) ? '#fff' : '#1a6b4a', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.15s' }}>
                            {!excludedEmails.includes(person.email) && <span style={{ color: '#fff', fontSize: 12, lineHeight: 1 }}>✓</span>}
                          </div>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 14, fontWeight: 600, color: excludedEmails.includes(person.email) ? '#b0a090' : '#2C1A0E' }}>{person.name}</div>
                            <div style={{ fontSize: 11, color: '#9B7B62' }}>{person.relationship}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <button
                    onClick={async () => {
                      ...
                      const res = await fetch('/api/request-prayer', {
                        ...
                        body: JSON.stringify({ userId: user.id, prayerText: prayerRequest, anonymous: prayerAnonymous, excludedEmails })
                      })
                      const data = await res.json()
                    ...
                    style={{ flex: 2, padding: '12px', borderRadius: 8, border: 'none', background: (prayerSending || !prayerRequest.trim()) ? '#ccc' : '#1a6b4a', color: '#fff', fontSize: 14, fontWeight: 'bold', cursor: 'pointer', fontFamily: 'Georgia, serif' }}
                  >
