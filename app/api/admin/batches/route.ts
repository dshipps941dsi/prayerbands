...
export async function GET(req: NextRequest) {
  ...
  if (batch) {
    ...
    return NextResponse.json({
      batch,
      total: bands.length,
      bands: bands.map(b => ({ band_id: b.band_id, theme: b.theme, color: b.color || '', nfc_url: b.nfc_url, outside_text: b.outside_text, inside_text: b.inside_text })),
    })
  }

  // No batch → summarize every batch. Page through all tagged rows and aggregate.
  const map = new Map<string, { batch: string; total: number; created: string; themes: Set<string>; colors: Set<string> }>()
  const PAGE = 1000
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await admin
      .from('bands')
      .select('batch, theme, color, created_at')
      .not('batch', 'is', null)
      .range(from, from + PAGE - 1)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    if (!data || data.length === 0) break
    for (const b of data) {
      ...
      if (!e) { e = { batch: b.batch, total: 0, created: b.created_at, themes: new Set(), colors: new Set() }; map.set(b.batch, e) }
      e.total++
  ...
  const batches = [...map.values()]
    .map(e => ({ batch: e.batch, total: e.total, created: e.created, themes: [...e.themes].sort(), colors: [...e.colors].sort() }))
