...
const STEPS = [
  ...
  { num: "3", img: "/home/step-3.png", title: "Receive Daily Verses", desc: "Get a new, personalized verse every day." },
  { num: "4", img: "/home/step-4.png", title: "Connect", desc: "Prayer partners, prayer circles, and encouragement." },
...
const FEATURES: { ico: string; title: string; desc: string; img?: string }[] = [
  ...
  { ico: "people", title: "Pray Together", desc: "Partners, circles, and encouragement.", img: "/home/pray-icon.png" },
...
// Rise-from-base: the child image starts fully below the container's base
// (clipped, off-screen) and rises into full view as the section scrolls up —
// mapped to a short scroll span so it lands quickly, like rising out of the
...
function useRiseFromBase(span = 1.0, hidden = 45) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      raf = 0;
      const rect = el.getBoundingClientRect(); // container isn't transformed, so no feedback
...
export default function HomePage() {
  ...
  // The daily phone starts hidden below the container's base and rises into view on scroll.
  const phoneRef = useRiseFromBase();
    ...
    <>
      ...
      <section id="daily" className="daily">
        <div className="daily-inner">
          <div ref={phoneRef} className="daily-phone">
...
const styles = `
  ...
  *,*::before,*::after { box-sizing: border-box; }
  ...
  /* Impact */
  .impact { background:var(--paper); padding:128px 0; }
  .impact-inner { display:grid; grid-template-columns:0.85fr 1.1fr 0.6fr; gap:48px; align-items:center; }
  .impact-copy .lead { font-size:1.08rem; max-width:520px; }
  .impact-card { background:#fff; border:1px solid var(--line); border-radius:18px; padding:42px 40px; box-shadow:0 18px 50px rgba(10,22,40,0.08); }
  ...
  .impact-row { display:flex; align-items:center; gap:16px; padding:13px 0; }
  .impact-dot { position:relative; z-index:1; width:12px; height:12px; border-radius:50%; background:var(--gold); flex-shrink:0; box-shadow:0 0 0 3px #fff, 0 0 0 6px rgba(200,169,110,0.18); }
  .impact-date { font-family:'Cinzel',serif; font-size:0.74rem; letter-spacing:0.06em; color:var(--goldT); width:58px; flex-shrink:0; }
  .impact-event { font-size:1.02rem; color:var(--ink); }
  .impact-stats-card { background:linear-gradient(180deg,#0E1E38,#0A1628); border:1px solid rgba(200,169,110,0.30); border-radius:18px; padding:36px 28px; box-shadow:0 14px 40px rgba(10,22,40,0.10); align-self:stretch; }
  ...
  .impact-stat-n { font-family:'Cormorant Garamond',serif; font-weight:600; font-size:2.8rem; color:#E2C98A; line-height:1; }
  .impact-stat-l { font-size:0.68rem; letter-spacing:0.08em; text-transform:uppercase; color:rgba(245,237,216,0.65); margin-top:6px; }
