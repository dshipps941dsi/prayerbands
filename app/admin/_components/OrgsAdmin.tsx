...
export default function AdminOrgs({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  return (
    <div className="pb-orgs-root" style={embedded ? { fontFamily: 'Inter, sans-serif' } : { fontFamily: 'Inter, sans-serif', background: C.pageBg, minHeight: '100vh', padding: 32 }}>
      ...
      `}</style>
      <div style={embedded ? undefined : { maxWidth: 1000, margin: '0 auto' }}>
        <div style={{ marginBottom: embedded ? 20 : 32 }}>
          {!embedded && <div style={{ marginBottom: 12 }}><PrayerBandsLogo size={34} color={C.gold} /></div>}
          {!embedded && <a href="/admin" style={{ color: C.goldText, fontSize: 13, textDecoration: 'none', fontFamily: 'Cinzel, serif', textTransform: 'uppercase', letterSpacing: '0.06em' }}>← Admin</a>}
          <h1 style={{ fontSize: embedded ? 22 : 30, fontWeight: 600, marginTop: embedded ? 0 : 8, marginBottom: 4, color: C.heading, fontFamily: 'Cormorant Garamond, Georgia, serif' }}>Church Accounts</h1>
          <p style={{ color: C.secondary, fontSize: 14, margin: 0 }}>Manage organizations, generate and assign bands.</p>
        </div>
