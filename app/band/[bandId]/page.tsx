  ...
  async function handleInitiateTransfer() {
    ...
    try {
      const res = await fetch('/api/initiate-transfer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ bandId, note: transferNote, recipientName: transferName, fromName: transferFrom }),
      })
      if (!res.ok) {
  ...
  const transferSheet = transferStep === 'sheet' ? (
    <div onClick={() => setTransferStep('idle')} style={{ position: 'fixed', inset: 0, background: 'rgba(44,24,16,0.4)', zIndex: 150, display: 'flex', alignItems: 'flex-end' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: CREAM, borderRadius: '20px 20px 0 0', padding: '28px 24px 48px', width: '100%', boxSizing: 'border-box' }}>
        ...
        <input value={transferFrom} onChange={e => setTransferFrom(e.target.value)} placeholder={myName ? `e.g. ${myName.split(' ')[0]}, or "Grandma Jo"` : 'Your name, as they should see it'} maxLength={80} style={{ display: 'block', width: '100%', padding: '12px 14px', border: '1px solid rgba(44,24,16,0.15)', borderRadius: 8, fontFamily: body, fontSize: 14, color: DARK, background: 'white', marginBottom: 14, outline: 'none', boxSizing: 'border-box' }} />
          ...
          <div style={{ margin: '24px 20px', background: `linear-gradient(160deg, ${NAVY}, ${NAVY_LT})`, borderRadius: 16, padding: '28px 24px', color: 'white', textAlign: 'center' }}>
            ...
            {status.transfer?.note && (
              <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 12, padding: '14px 16px', fontFamily: body, fontSize: 14, fontStyle: 'italic', lineHeight: 1.6, marginBottom: 20, textAlign: 'left' }}>
                "{status.transfer.note}"
                {status.senderName && <div style={{ marginTop: 8, textAlign: 'right', fontStyle: 'normal', fontFamily: serif, fontWeight: 700 }}>&mdash; {status.senderName}</div>}
              </div>
            )}
